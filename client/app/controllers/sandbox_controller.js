$j.c({Sandbox: {
        ...
        index: function() {
          ...
          $j.m.Post.find_all(function(posts){
            $('#list').append ('<ul>');
            $(document).data('posts',posts);
            for( i in posts){
              var post = posts[i]['post'];
              $('#list').append('<li><a href="#" alt="'+i+'">'+post["title"]+'</a></li>');
            }
            $('#list').append('</ul>');
          });

          // $j.m.Post.initialize(function(data){
          //   $('#post2').form_for(data).
          //     label('title','Título del post').text_field('title').
          //     label('body', 'Cuerpo del post').text_area('body');
          // })
          alert('ho1111la');
          
          $('#post1').form_for('post').
             label('title','Titulo del post').text_field('title').
             label('body', 'Cuerpo del texto').text_area('body').
             submit_tag();
           
          $('#list a').live('click', function(){
            var posts = $(document).data('posts');
            var id = eval($(this).attr('alt'));
            $('#post1 form').set_model(posts[id]);
            return false;
          });
